...
export class HomeComponent {
  ...
  ngAfterViewInit() {
    ...
    this.typeText("text-line-1", this.text1, 0, () => {
      ...
      }, 500); // Delay before second line starts typing
    });
  }
  ...
  typeText(elementId: string, text: string, index: number, callback?: () => void) {
    if (index < text.length) {
      document.getElementById(elementId)!.innerHTML += text.charAt(index);
      setTimeout(() => this.typeText(elementId, text, index + 1, callback), this.typingSpeed);
    } else if (callback) {
      callback();
    }
  }
